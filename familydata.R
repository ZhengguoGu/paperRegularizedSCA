...
DS3 <- DS3[-which(DS3[, c(352:367)] == 9 | is.na(DS3[, c(352:367)]), arr.ind = T)[,1], ]  #the rows containing missing values are removed
...
DS3 <- DS3[-which(DS3[, c(368:371)] == 9 | is.na(DS3[, c(368:371)]), arr.ind = T)[,1], ]
...
DS3 <- DS3[-which(DS3[, c(550:553)] == 9 | is.na(DS3[, c(550:553)]), arr.ind = T)[,1], ]
...
DS3 <- DS3[-which(DS3[, c(555:568)] == 9 | is.na(DS3[, c(555:568)]), arr.ind = T)[,1], ]
DS3[, c(556:562, 565, 566)] <- 5 -  DS3[, c(556:562, 565, 566)]

# for the remaining questionnaires, we moved the missing values
DS3 <- DS3[-which(DS3[, c(379:404)] == 9 | is.na(DS3[, c(379:404)]), arr.ind = T)[,1], ]
DS3 <- DS3[-which(DS3[, c(441:460)] == 9 | is.na(DS3[, c(441:460) ]), arr.ind = T)[,1], ]
DS3 <- DS3[-which(DS3[, c(441:460)] == -8, arr.ind = T)[,1], ]
DS3 <- DS3[-which(DS3[, c(540:549)] == 9 | is.na(DS3[, c(540:549)]), arr.ind = T)[,1], ]
...
####################################
# now DS4
DS4 <- DS4[-which(DS4[, c(176:183)] == 9 | is.na(DS4[, c(176:183)]), arr.ind = T)[,1], ]
DS4[, 176] <- 3 - DS4[, 176]  #recoding, answer category 0~3

DS4 <- DS4[-which(DS4[, 304] == 99 | is.na(DS4[, 304])), ] #academic performance
DS4 <- DS4[-which(DS4[, 304] == 9 | is.na(DS4[, 304])), ]

DS4 <- DS4[-which(DS4[, c(326:342)] == 9 | is.na(DS4[, c(326:342)]), arr.ind = T)[,1], ]  #extracurricular

DS4 <- DS4[-which(DS4[, c(359:367)] == 9 | is.na(DS4[, c(359:367)]), arr.ind = T)[,1], ]  #Importance of friendship

DS4 <- DS4[-which(DS4[, c(381:397)] == 9 | is.na(DS4[, c(381:397)]), arr.ind = T)[,1], ]  #Self image
DS4[, c(383,384, 386:388, 390:394, 396,397)] <- 4 - DS4[, c(383,384, 386:388, 390:394, 396,397)] #reverse coding

DS4 <- DS4[-which(DS4[, c(399:418)] == 9 | is.na(DS4[, c(399:418)]), arr.ind = T)[,1], ] #happiness
DS4[, c(399, 400, 402, 403, 405:411, 413, 414, 416:418)] <- 3 - DS4[, c(399, 400, 402, 403, 405:411, 413, 414, 416:418)] 

DS4 <- DS4[-which(DS4[, c(468:480)] == 9 | is.na(DS4[, c(468:480)]), arr.ind = T)[,1], ] #confidence about future
